 ...
 * @since 2023/10/7
 */
public class OdpsTask {

    private static final Logger LOG = LoggerFactory.getLogger(OdpsTask.class);
    ...
    static {
        prop = new Properties();
        try {
            prop.load(OdpsTask.class.getClassLoader().getResourceAsStream("odps-core.properties"));
        } catch (IOException e) {
        ...
        try(BufferedReader br = new BufferedReader(new InputStreamReader(new FileInputStream(dataPath)))){
            ...
            while (null != (temp=br.readLine())){
                ...
                if(arr.length==3){
                    ...
                    com.aliyun.odps.data.Record record = new WritableRecord(columns);
                    record.set(0,arr[0]);
                    record.set(1,arr[1]);
                    record.set(2,arr[2]);
    ...
    private void execute() throws OdpsException {
        ...
        job.setResources("odsp-mr-1.0-SNAPSHOT.jar");
